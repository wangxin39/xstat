...
<script type="text/javascript">
	function check(){
		...
		document.getElementById("check").action="editIssueProcess.do";
...
<body>
<div class="cright">
  <div class="title font16h">修改期次信息</div>
  <div class="container">	
	<s:form action="editIssueProcess">
	<s:textfield label="期次" name="issue" value="%{#session.ISSUEINFOEDIT.issue}"></s:textfield>
	<tr>
	<td class="tdLabel"><label class="label">客户名称:</label></td>
	<td>
	<s:if test="#session.GLOBALCLIENTINFOLIST != null">
	<s:select label="客户名称" list="%{#session.GLOBALCLIENTINFOLIST}" name="clientID" value="%{#session.ISSUEINFOEDIT.clientID}" listValue="company" listKey="clientID" onchange="selectClient(this);" theme="simple"></s:select>
	</s:if>
	<s:else>
	...
	<s:select id="projectID" label="项目名称" list="%{#session.PROJECTSELECT}" name="projectID" listValue="name" value="%{#session.ISSUEINFOEDIT.projectID}" listKey="projectID" theme="simple"></s:select>
	</s:if>
	<s:else>
	...
	<s:select id="inquisitionID" label="调查问卷" list="%{#session.INQUISITIONSELECT}" name="inquisitionID" listValue="title" value="%{#session.ISSUEINFOEDIT.inquisitionID}" listKey="inquisitionID" theme="simple"></s:select>
	</s:if>
	<s:else>
	...
	<s:radio name="isFirst" label="比较类型" list="#{'1':'当年第一期','2':'不是第一期'}" value="%{#session.ISSUEINFOEDIT.isFirst}" required="true"/>
	<tr>
	<tr>
	<td>&nbsp;</td>
	<td align="right">
