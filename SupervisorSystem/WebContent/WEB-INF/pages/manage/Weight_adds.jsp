...
<body>
<div class="cright">
  <div class="title font16h">添加权重信息</div>
  <div class="container">	
	<s:actionerror/>
	<s:actionmessage/>
	<s:form action="addWeightProcess" method="post">
	<tr>
	<td class="tdLabel"><label class="label">调查问卷:</label></td>
	<td>
	<s:if test="null != inquisitionInfoList">
	<s:select id="inquisitionID" label="调查问卷" list="%{inquisitionInfoList}" name="inquisitionID" listKey="inquisitionID"  onchange="selectPart(this);" listValue="title" headerKey="-1" theme="simple"></s:select>
	</s:if>
	<s:else>
	...
	<s:select id="partID" label="部分名称" list="%{partInfoList}" name="partID" listKey="partID" listValue="name" onchange="selectQuestion(this);" headerKey="-1" theme="simple"></s:select>
	</s:if>
	<s:else>
	...
	<s:select label="部分问题" list="%{partQuestionInfoList}" name="partQuestionID" listValue="fieldName" listKey="partQuestionID" headerKey="-1" theme="simple"></s:select>
	</s:if>
	<s:else>
	...
	<s:select label="问题选项" list="%{selectInfoList}" name="optionID" listValue="chinaOption" listKey="optionID" headerKey="-1"></s:select>
	</s:if>
	<s:else>
	<s:select label="问题选项" name="optionID" list="{}"></s:select>	
	</s:else>
	<s:textfield label="权重" name="weight"></s:textfield>
	<tr>
	<td>&nbsp;</td>
