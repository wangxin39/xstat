...
<body>
<div class="cright">
  <div class="title font16h">修改视频信息</div>
  <div class="container">	
	<s:form action="editVideoProcess" namespace="/manage" method="post">
	<s:textfield label="连接地址" name="url" value="%{#session.VIDEOEDIT.url}"></s:textfield>
	<s:textfield label="名称" name="name" value="%{#session.VIDEOEDIT.name}"></s:textfield>
	<s:textfield name="englishName" value="%{#session.VIDEOEDIT.englishName}" label="英文名称"></s:textfield>
	
	<tr>
	<td class="tdLabel"><label class="label">客户名称:</label></td>
	<td>
	<s:if test="#session.GLOBALCLIENTINFOLIST!=null">
	<s:select list="%{#session.GLOBALCLIENTINFOLIST}" label="客户名称" name="clientID" value="%{#session.VIDEOEDIT.clientID}" listValue="company" listKey="clientID" onchange="selectClient(this);" theme="simple" headerKey="-1"></s:select>
	</s:if>
	<s:else>
	...
	<s:select id="informantID" label="调查对象" list="%{#session.INFORMANTLIST}" name="informantID" value="%{#session.VIDEOEDIT.informantID}" listValue="name" listKey="informantID" headerKey="-1" theme="simple"></s:select>
	</s:if>
	<s:else>
