...
<div class="cright">
  ...
  <div class="container">	
	...
	<s:form action="addReportTemplateProcess" enctype="multipart/form-data"  method="POST">
	<s:textfield label="名称" name="name" ></s:textfield>
	<s:if test="#session.TYPE !=null">
	<s:select list="%{#session.TYPE}" name="type" label="类型"></s:select>
	</s:if>
	<s:else>
	<s:select list="%{}" name="type" label="类型"></s:select>
	</s:else>
	<s:file label="文件名称" name="upload" ></s:file>
	<s:textfield label="存储路径" name="path" ></s:textfield>
