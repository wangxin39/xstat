...
<body>
<div class="cright">
  <div class="title font16h">添加报告信息</div>
  <div class="container">	
	<s:form action="addReportProcess">
	<s:actionerror/>
<s:actionmessage/>
	<tr>
	...
	<s:select list="%{#session.THEMELIST}" name="themeID" label="主题名称" listKey="themeID" listValue="title" headerKey="-1"></s:select>
	</s:if>
	<s:else>
	<s:select list="{}" name="themeID" label="主题名称"></s:select>
	</s:else>
	<s:if test="#session.REPORTTYPE != null">
	<s:select list="%{#session.REPORTTYPE}" name="reportType" label="报告类型"></s:select>
	</s:if>
	<s:else>
	<s:select label="报告类型" list="{}" name="reportType"></s:select>
	</s:else>
	<s:textfield label="检测名称" name="checkName"></s:textfield>
	<s:textfield label="报告中文名称" name="reportTitle"></s:textfield>
	<s:textfield label="报告英文名称" name="enReportTitle"></s:textfield>
	<s:textfield label="季度标题" name="quarterTitle"></s:textfield>
	<s:textfield label="报告页数" name="pageCount"></s:textfield>
	<tr>
	<td>&nbsp;</td>
