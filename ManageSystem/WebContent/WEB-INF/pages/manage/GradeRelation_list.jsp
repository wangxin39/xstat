...
function updatePanel(num)
    ...
    function processResponse(response)
	{
		...
		html +='<tr><td class=\"ttitle\"><strong class=\"font12red\">编号</strong></td><td class=\"ttitle\"><strong class=\"font12red\">客户</strong></td><td class=\"ttitle\"><strong class=\"font12red\">期次</strong></td><td class=\"ttitle\"><strong class=\"font12red\">调查对象</strong></td><td class=\"ttitle\"><strong class=\"font12red\">评分标准</strong></td><td class=\"ttitle\"><strong class=\"font12red\">操作</strong></td></tr>';
		for(i=0;i<length;i++){
			html += '<tr><td>'+o.pageList[i].gradeRelationID+'&nbsp;</td>';
			html += '<td>'+o.clientList[i]+'&nbsp;</td>';	
			html += '<td>'+o.issueList[i]+'&nbsp;</td>';	
			html += '<td>'+o.informantList[i]+'&nbsp;</td>';	
			html += '<td>'+o.gradeStdList[i]+'&nbsp;</td>';	
			html += '<td><input type="button" onClick="ConfirmButton(\'editGradeRelationInput.do?GradeRelation='+o.pageList[i].gradeRelationID+'\',\'是否确认修改评分关系信息!\');" value="修改">';
			html += '<input type="button" onClick="ConfirmButton(\'deleteGradeRelationProcess.do?GradeRelation='+o.pageList[i].gradeRelationID+'\',\'是否确认删除评分关系信息!\');" value="删除">';
			html += '</td></tr>';				
		}
		html += '</table>';
	...
	function showDetail(id) {
		window.open('detailGradeRelationStore.do?gradeRelationID='+id,'详细信息','width=500,height=400,scrollbars=yes,status=yes,left=100,top=100');		
	}	
...
<body>
<div class="cright">
  <div class="title font16h">评分关系管理</div>
  <div class="container">	
	<table width="100%" border="0" cellspacing="10">
	  	<tr>
    	  <td align="right"><input type="button" name="button" value="增加新的评分关系" class="button" onclick="GoURL('addGradeRelationInputs.do');"/>&nbsp;&nbsp;&nbsp;&nbsp;</td>
	     </tr>
	 </table>
	<div id="tablecontent">
	<table  width="100%" border="1" cellpadding="0" cellspacing="0" bordercolor="#FFFFFF" bgcolor="#cccccc">
		...
		<s:iterator value="%{pageList}" status="stat">
			<tr>
				<td><s:property value="gradeRelationID" />&nbsp;</td>
				...
				<td>
					<input type="button" onClick="ConfirmButton('editGradeRelationInput.do?gradeRelationID=<s:property value="%{gradeRelationID}"/>','是否确认修改评分关系信息!');" value="修改">
					<input type="button" onClick="ConfirmButton('deleteGradeRelationProcess.do?gradeRelationID=<s:property value="%{gradeRelationID}"/>','是否确认删除评分关系信息!');" value="删除">
				</td>
		</s:iterator>
	</table>
	</div>
