...
<body>
<div class="cright">
  <div class="title font16h">添加评语信息</div>
  <div class="container">	
<s:form action="addCritiqueProcess">
<tr>
...
<s:select id="projectID" label="项目名称" list="%{#session.GLOBALPROJECTLIST}" name="projectID" listValue="name" listKey="projectID" headerKey="-1" theme="simple"></s:select>
</s:if>
<s:else>
...
<s:select id="informantID" label="调查对象" list="%{#session.GLOBALINFORMANTLIST}" name="informantID" listValue="name" listKey="informantID" headerKey="-1" theme="simple"></s:select>
</s:if>
<s:else>
...
<s:select id="inquisitionID" label="调查问卷" list="%{#session.GLOBALINQUISITIONLIST}" name="inquisitionID" listValue="title" listKey="inquisitionID" headerKey="-1" onchange="selectInquisition(this);" theme="simple"></s:select>
</s:if>
<s:else>
...
<s:select id="issueID" label="期次" list="%{#session.GLOBALISSUELIST}" name="issueID" listValue="issue" listKey="issueID" headerKey="-1" theme="simple"></s:select>
</s:if>
<s:else>
...
<s:select id="partID" label="部分名称" list="%{#session.GLOBALPARTLIST}" name="partID" listValue="name" listKey="partID" headerKey="-1" theme="simple"></s:select>
</s:if>
<s:else>
...
<s:textarea label="优点" name="strengths" cols="50" rows="8"></s:textarea>
<s:textarea label="英文优点" name="enStrengths" cols="50" rows="8"></s:textarea>
<s:textarea label="缺点" name="weaknesses" cols="50" rows="8"></s:textarea>
<s:textarea label="英文缺点" name="enWeaknesses" cols="50" rows="8"></s:textarea>
<s:textarea label="总结建议" name="conclusion" cols="50" rows="8"></s:textarea>
<s:textarea label="英文总结建议" name="enConclusion" cols="50" rows="8"></s:textarea>
<tr>
<td>&nbsp;</td>
