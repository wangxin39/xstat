...
function updatePanel(num)
    ...
    function processResponse(response)
	{
		...
		html +='<tr><td class=\"ttitle\"><strong class=\"font12red\">编号</strong></td><td class=\"ttitle\"><strong class=\"font12red\">标题</strong></td><td class=\"ttitle\"><strong class=\"font12red\">描述</strong></td><td class=\"ttitle\"><strong class=\"font12red\">操作</strong></td></tr>';
		for(i=0;i<length;i++){
			html += '<tr><td>'+o.pageList[i].gradeStdID+'&nbsp;</td>';
			html += '<td>'+o.pageList[i].title+'&nbsp;</td>';	
			html += '<td>'+o.pageList[i].description+'&nbsp;</td>';	
			html += '<td><input type="button" onClick="ConfirmButton(\'editGradeStdInput.do?gradeStdID='+o.pageList[i].gradeStdID+'\',\'是否确认修改评分标准信息!\');" value="修改">';
			html += '<input type="button" onClick="ConfirmButton(\'deleteGradeStdProcess.do?gradeStdID='+o.pageList[i].gradeStdID+'\',\'是否确认删除评分标准信息!\');" value="删除">';
			html += '</td></tr>';				
		}
		html += '</table>';
...
<body>
<div class="cright">
  <div class="title font16h">评分标准管理</div>
  <div class="container">	
    <table width="100%" border="0" cellspacing="10">
    	<tr>
     	  <td align="right"><input type="button" name="button" value="增加新的评分标准" class="button" onclick="GoURL('addGradeStdInputs.do');"/>
     	    &nbsp;&nbsp;&nbsp;&nbsp;</td>
      </tr>
    </table>
	<div id="tablecontent">
	<table  width="100%" border="1" cellpadding="0" cellspacing="0" bordercolor="#FFFFFF" bgcolor="#cccccc">
		...
		<s:iterator value="%{pageList}" status="stat">
			<tr>
				<td><s:property value="gradeStdID" />&nbsp;</td>
				<td><s:property value="title" />&nbsp;</td>
				<td><s:property value="description" />&nbsp;</td>
				<td>
					<input type="button" onClick="ConfirmButton('editGradeStdInput.do?gradeStdID=<s:property value="%{gradeStdID}"/>','是否确认修改评分标准信息!');" value="修改">
					<input type="button" onClick="ConfirmButton('deleteGradeStdProcess.do?gradeStdID=<s:property value="%{gradeStdID}"/>','是否确认删除评分标准信息!');" value="删除">
				</td>
		</s:iterator>
	</table>
	</div>
	<div id="pagination">第<s:property value="pu.currentPage"/>页/总<s:property value="pu.totalPageNum" />页 		
