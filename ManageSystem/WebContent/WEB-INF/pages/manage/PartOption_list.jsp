...
function updatePanel(num)
    ...
    function processResponse(response)
	{
		...
		html +='<tr><td class=\"ttitle\"><strong class=\"font12red\">编号</strong></td><td class=\"ttitle\"><strong class=\"font12red\">选项信息</strong></td><td class=\"ttitle\"><strong class=\"font12red\">显示顺序</strong></td><td class=\"ttitle\"><strong class=\"font12red\">显示编号</strong></td><td class=\"ttitle\"><strong class=\"font12red\">操作</strong></td></tr>';
		for(i=0;i<length;i++){
			html += '<tr><td>'+o.pageList[i].partSelectID+'</td>';
			html += '<td><a href="#" OnClick="javascript:showDetail(\''+o.pageList[i].partSelectID+'\')">'+o.selectList[i]+'</a></td>';	
			html += '<td>'+o.pageList[i].showOrder+'</td>';			
			html += '<td>'+o.pageList[i].showNum+'</td>';						
			html += '<td><input type="button" onClick="ConfirmButton(\'editPartOptionInput.do?partSelectID='+o.pageList[i].partSelectID+'\',\'是否确认修改部分选项问题信息!\');" value="修改">';
			html += '<input type="button" onClick="ConfirmButton(\'deletePartOptionProcess.do?partSelectID='+o.pageList[i].partSelectID+'\',\'是否确认删除部分选项问题信息!\');" value="删除">';
			html += '</td></tr>';				
		}
		html += '</table>';
	...
	function showDetail(id) {
		window.open('detailPartOptionStore.do?partSelectID='+id,'详细信息','width=500,height=400,scrollbars=yes,status=yes,left=100,top=100');		
	}	
...
<body>
<div class="cright">
  <div class="title font16h">部分选项问题管理</div>
  <div class="container">	
	    <table width="100%" border="0" cellspacing="10">
	    	<tr>
	     	  <td align="right"><input type="button" name="button" value="增加新的部分选项问题" class="button" onclick="GoURL('addPartOptionInputs.do');"/>
	     	    &nbsp;&nbsp;&nbsp;&nbsp;</td>
	      </tr>
	    </table>
	<div id="tablecontent">
	<table  width="100%" border="1" cellpadding="0" cellspacing="0" bordercolor="#FFFFFF" bgcolor="#cccccc">
		...
		<s:iterator value="%{pageList}" status="stat">
			<tr>
				<td><s:property value="partSelectID" /></td>
				<td><a href="#" OnClick="javascript:showDetail('<s:property value="partSelectID"/>')"><s:property value="selectList[#stat.index]" /></a></td>
				<td><s:property value="showOrder" />&nbsp;</td>
				<td><s:property value="showNum" />&nbsp;</td>
				<td>
					<input type="button" onClick="ConfirmButton('editPartOptionInput.do?partSelectID=<s:property value="%{partSelectID}"/>','是否确认修改部分选项问题信息!');" value="修改">
					<input type="button" onClick="ConfirmButton('deletePartOptionProcess.do?partSelectID=<s:property value="%{partSelectID}"/>','是否确认删除部分选项问题信息!');" value="删除">
				</td>
			</tr>
		</s:iterator>
	</table>
