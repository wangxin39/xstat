...
<div class="cright">
  ...
  <div class="container">	
	<s:actionerror/>
	<s:actionmessage/>
	<s:form action="addWeightProcess" method="post" namespace="/manage">
	<tr>
	<td class="tdLabel"><label class="label">调查问卷:</label></td>
	<td>
	<s:if test="null != inquisitionInfoList">
	<s:select id="inquisitionID" label="调查问卷" list="%{inquisitionInfoList}" name="inquisitionID" listKey="inquisitionID"  onchange="selectPart(this);" listValue="title" headerKey="-1" theme="simple" onchange="selectPart(this);"></s:select>
	</s:if>
	<s:else>
	...
	<s:select id="partID" label="部分" list="%{partInfoList}" name="partID" listKey="partID" listValue="name" onchange="selectQuestion(this);" headerKey="-1" theme="simple"></s:select>
	</s:if>
	<s:else>
	<s:select id="partID" label="部分" name="partID" list="{}" theme="simple"></s:select>
	</s:else>
	</div>
	</td>
	</tr>
