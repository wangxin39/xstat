...
<script type="text/javascript">
	function updatePanel(num,total)
	{
		//请求的地址
		var url = 'ajaxAddress.do';
		var pars = 'num='+num+'&total='+total;
		var keyword = $('p_keyword').value;
		if(keyword != '') {
    ...
    function ajaxResponse(response)
	{
		...
		html +='<tr><td class=\"ttitle\"><strong class=\"font12red\">编号</strong></td><td class=\"ttitle\"><strong class=\"font12red\">用户名称</strong></td><td class=\"ttitle\"><strong class=\"font12red\">联系电话</strong></td><td class=\"ttitle\"><strong class=\"font12red\">手机号码</strong></td><td class=\"ttitle\"><strong class=\"font12red\">QQ</strong></td><td class=\"ttitle\"><strong class=\"font12red\">操作</strong></td></tr>';
		for(i=0;i<length;i++){
			html += '<tr>';
			html += '<td>'+o.pageList[i].addressID+'</td>';	
			html += '<td>'+o.acList[i]+'</td>';
			html += '<td>'+o.pageList[i].name+'</td>';
			html += '<td>'+o.pageList[i].telephone+'</td>';
			html += '<td>'+o.pageList[i].mobile+'</td>';
			html += '<td>'+o.pageList[i].QQ+'</td>';
			html += '<td><input type="button" onClick="ConfirmButton(\'editAddressInput.do?addressID='+o.pageList[i].addressID+'\',\'是否确认修改通讯录信息!\');" value="修改"/>';
			html += '<input type="button" onClick="ConfirmButton(\'deleteAddressProcess.do?addressID='+o.pageList[i].addressID+'\',\'是否确认删除通讯录信息!\');" 	value="删除"/>';					
			html += '</td></tr>';				
		}
		html += '</table>';
...
<body>
<div class="cright">
  <div class="title font16h">通讯录管理</div>
  <div class="container">	
	    <table width="100%" border="0" cellspacing="10">
	    	<tr>
	     	  <td align="right"><input type="button" name="button" value="增加新通讯录" class="button" onclick="GoURL('addAddressInputs.do');"/>
	     	    &nbsp;&nbsp;&nbsp;&nbsp;</td>
	      </tr>
	    </table>
	<div id="tablecontent">
	<table  width="100%" border="1" cellpadding="0" cellspacing="0" bordercolor="#FFFFFF" bgcolor="#cccccc">
		<tr>
			...
			<td class="ttitle"><strong class="font12red">分类名称</strong></td>
			<td class="ttitle"><strong class="font12red">用户姓名</strong></td>
			<td class="ttitle"><strong class="font12red">联系电话</strong></td>
			<td class="ttitle"><strong class="font12red">手机号码</strong></td>
			<td class="ttitle"><strong class="font12red">QQ</strong></td>
			<td class="ttitle"><strong class="font12red">操作</strong></td>
		</tr>
		<s:iterator value="%{pageList}" status="stat">
			<tr>
				<td><s:property value="addressID" /></td>
				<td><s:property value="acList[#stat.index]" /></td>
				<td><s:property value="name" /></td>
				<td><s:property value="telephone" /></td>
				<td><s:property value="mobile" /></td>
				<td><s:property value="QQ" /></td>
				<td>
					<input type="button" onClick="ConfirmButton('editAddressInput.do?addressID=<s:property value="%{addressID}"/>','是否确认修改通讯录信息!');"	value="修改"> 
					<input type="button" onClick="ConfirmButton('deleteAddressProcess.do?addressID=<s:property value="%{addressID}"/>','是否确认删除通讯录信息!');"	value="删除">
				</td>
			</tr>
		</s:iterator>
	</table>
