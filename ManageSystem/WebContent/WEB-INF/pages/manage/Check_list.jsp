...
function updatePanel(num)
    ...
    function processResponse(response)
	{
		...
		html +='<tr><td class=\"ttitle\"><strong class=\"font12red\">改进编号</strong></td><td class=\"ttitle\"><strong class=\"font12red\">原因</strong></td><td class=\"ttitle\"><strong class=\"font12red\">操作</strong></td></tr>';
		for(i=0;i<length;i++){
			html += '<tr><td>'+o.pageList[i].checkDetailID+'</td>';
			html += '<td><a href="#" OnClick="javascript:showDetail(\''+o.pageList[i].checkDetailID+'\')">'+o.pageList[i].what+'</a></td>';	
			html += '<td><input type="button" onClick="ConfirmButton(\'editCheckInput.do?checkDetailID='+o.pageList[i].checkDetailID+'\',\'是否确认修改改进信息!\');" value="修改"/>'; 
			html += '<input type="button" onClick="ConfirmButton(\'deleteCheckProcess.do?checkDetailID='+o.pageList[i].checkDetailID+'\',\'是否确认删除改进信息!\');" value="删除"/>';
			html += '</td></tr>';
	...
	function showDetail(id) {
		window.open('detailCheckStore.do?checkDetailID='+id,'详细信息','width=500,height=400,scrollbars=yes,status=yes,left=100,top=100');		
	}	
	
	function selectClient(opt)
	{//这个方法中只能有两个参数(url,pars);
		//请求的地址
		var url = 'selectCheck.do';
		
		var iID = opt.value;//获取传入的客户值
		
		var pars = 'inquisitionID='+iID;
    ...
    function clientResponse(response)
	{
		...
		var html = '<select name=\'issueID\' id=\'issueID\'>';
		var length = o.issueList.length;
		for(i=0;i<length;i++){
...
<body>
<div class="cright">
  <div class="title font16h">改进管理</div>
  <div class="container">	
	    <table width="100%" border="0" cellspacing="10">
	    	<tr>
	     	  <td align="right"><input type="button" name="button" value="增加新的改进" class="button" onclick="GoURL('addCheckInputs.do');"/>
	     	    &nbsp;&nbsp;&nbsp;&nbsp;</td>
	      </tr>
	    </table>
	<s:form action="listCheck">
	<s:if test="#session.GLOBALCLIENTINFOLIST != null">
	<s:select label="客户名称" list="%{#session.GLOBALCLIENTINFOLIST}" name="clientID" listValue="company" value="%{#request.CLIENTVALUE}" listKey="clientID" headerKey="-1"></s:select>
	</s:if>
	<s:else>
...
<div id="tablecontent">
	<table  width="100%" border="1" cellpadding="0" cellspacing="0" bordercolor="#FFFFFF" bgcolor="#cccccc">
		<tr>
			<td class="ttitle"><strong class="font12red">改进编号</strong></td>
			<td class="ttitle"><strong class="font12red">原因</strong></td>
			<td class="ttitle"><strong class="font12red">操作</strong></td>
		</tr>
		<s:iterator value="%{pageList}" status="stat">
			<tr>
				<td><s:property value="checkDetailID" /></td>
				<td><a href="#" OnClick="javascript:showDetail('<s:property value="checkDetailID"/>')"><s:property value="what"/></a></td>
				<td>
					<input type="button" onClick="ConfirmButton('editCheckInput.do?checkDetailID=<s:property value="%{checkDetailID}"/>','是否确认修改改进信息!');" value="修改"> 
					<input type="button" onClick="ConfirmButton('deleteCheckProcess.do?checkDetailID=<s:property value="%{checkDetailID}"/>','是否确认删除改进信息!');"	value="删除">
				</td>
			</tr>
		</s:iterator>
	</table>
