...
<body>
<div class="cright">
  <div class="title font16h">添加改进信息</div>
  <div class="container">	
	<s:form action="addCheckProcess">
	<tr>
	...
	<s:select label="调查问卷" list="%{#session.INQUISITIONLIST}" name="inquisitionID" listValue="title" listKey="inquisitionID" headerKey="-1" theme="simple"></s:select>
	</s:if>
	<s:else>
	...
	<s:select id="informantID" label="项目名称" list="%{#session.PROJECTLIST}" name="projectID" listValue="name" listKey="projectID" headerKey="-1" theme="simple"></s:select>
	</s:if>
	<s:else>
	...
	<s:select label="类型" list="%{#session.TYPE}" name="type"></s:select>
	</s:if>
	<s:else>
	<s:select label="类型" list="{}" name="type"></s:select>
	</s:else>
	<s:textarea label="原因" name="what" cols="50" rows="8"></s:textarea>
	<s:textarea label="英文原因" name="enWhat" cols="50" rows="8"></s:textarea>
	<tr>
	<td>&nbsp;</td>
