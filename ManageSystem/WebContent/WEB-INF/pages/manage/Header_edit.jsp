...
<div class="cright">
  ...
  <div class="container">	
	...
	<s:textfield label="显示顺序" name="showOrder" value="%{#session.HEADEREDIT.showOrder}" size="20"></s:textfield>
	<s:select list="#{'1':'选项模板','2':'表头选项'}" label="选项类型" name="selectType" value="%{#session.HEADEREDIT.selectType}"></s:select>
	<s:if test="#session.OPTIONTEMPLATEINFOLIST != null">
	<s:select list="%{#session.OPTIONTEMPLATEINFOLIST}" label="选项模板" name="optionTemplateID" listValue="title" listKey="optionTemplateID" headerKey="-1" value="%{#session.HEADEREDIT.optionTemplateID}"></s:select>	
	</s:if>
	<s:else>
	<s:select list="%{}" label="选项模板" name="optionTemplateID" value="%{#session.HEADEREDIT.optionTemplateID}"></s:select>	
	</s:else>
	<tr>
	<td>&nbsp;</td>
	<td align="right">
